= Health Check
:description: pass:q[Health Check provides `ping()` tests for the health of the network and the cluster.]
...
Working in distributed environments is hard.
...
[source,rust]
----
include::devguide:example$src/diagnostics.rs[tag=ping,indent=0]
----
