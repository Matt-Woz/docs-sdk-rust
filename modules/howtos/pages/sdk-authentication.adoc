...
[source,rust]
----
include::devguide:example$src/auth.rs[tag=basic,indent=0]
----
...
== Authenticating a Rust Client by Certificate
...
That is, a cluster certificate has been created and installed on the server, a client certificate has been created, and it is accessible along with the cluster's certificate.

[source,rust]
----
include::devguide:example$src/auth.rs[tag=certificate,indent=0]
----
