...
=== Logger example with env_logger
...
You must also then register a global logger to consume the log messages.
The simplest example of using `env_logger` is simply to add the following before executing any Couchbase SDK code:
...
[source,rust]
----
include::devguide:example$src/logging.rs[tag=env-logger-extended,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/diagnostics.rs[tag=cluster-wait-until-ready,indent=0]
----
