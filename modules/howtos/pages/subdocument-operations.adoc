...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=get,indent=0]
----

.Check existence of Sub-Document path
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=exists,indent=0]
----

NOTE: LookupInResult has an `exists` method, but this should not be confused with the exists __operation__.
The `exists` method is used to check if anything was returned by the server for a given operation.
The result of the exists operation should be checked with `contentAs[Boolean]`, as in the example.
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=insert,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=combine_mutate,indent=0]
----

NOTE: `mutateIn` is an _atomic_ operation.
If any single operation fails, then the entire document is left unchanged.
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_append,indent=0]
----

[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_prepend,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_document,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_upsert,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_add_unique,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=array_insert,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=counter_increment,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=counter_decrement,indent=0]
----
...
NOTE: Multiple Sub-Document operations are formed into a single command packet, which is then executed atomically on the server.
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=create_path,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=concurrent_subdoc_operations,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=cas,indent=0]
----

== Durability

For more information see xref:concept-docs:durability-replication-failure-considerations.adoc#durable-writes[Durable Writes], which uses the concept of xref:{version-server}@server:learn:data/durability.adoc#majority[majority] to indicate the number of configured Data Service nodes to which commitment is required:

[source,rust]
----
include::devguide:example$src/subdocument.rs[tag=durability,indent=0]
----

== Error Handling

Sub-Document operations have their own set of errors.
When programming with Sub-Document, be prepared for any of the full-document errors (such as _DocumentNotFoundException_) as well as special Sub-Document errors which are received when certain constraints are not satisfied.
...
If you need to combine both JSON _and_ path-syntax literals you can do so by escaping the component from any JSON string characters (e.g. a quote or backslash) and then encapsulating it in backticks (`pass:c[`path`]`).
