...
What's the point of a fast and rustble database if it's not easy to develop for?
...
[source,rust]
----
include::devguide:example$rust/KvOperations.rs[indent=0,tag=upsert]
----
--
...
[source,rust]
----
include::devguide:example$rust/Queries.rs[tag=simple,indent=0]
----
--
...
[source,rust]
----
include::devguide:example$rust/FTS.rs[tag=vector1,indent=0]
----
--
====
...
.Installing the SDK via Rust Build Tool
[source,sbt,subs="normal, attributes"]
----
libraryDependencies += "com.couchbase.client" %% "rust-client" % "{sdk_current_version}"
----

The Rust SDK is provided with builds for Rust 2.12, 2.13, and 3.
`%%` takes care of selecting the right version in Rust Build Tool.
If you are using another build tool such as Maven or Gradle, then specify `rust-client_2.12`, `rust-client_2.13`, or `rust-client_3`, as appropriate.
The Rust 3 build can be used from applications compiled with Rust 3.3 through 3.7 inclusive, and even Rust 2.13.
