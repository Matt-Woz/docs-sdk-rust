...
:forum-link: https://www.couchbase.com/forums/c/other-product/13
// todo - Rust forum!
:page-partial:
...
// https://github.com/couchbase/docs-sdk-rust/tree/release/1.0/modules/devguide/examples/
//
// It can be seen built at
// https://docs.couchbase.com/rust-sdk/1.0/hello-world/start-using-sdk.html


[abstract]
{description}

Couchbase has a simple interface for creating and modifying records in a document, based upon the *collection* into which the documents are organized.
You can read more about data modeling <<data-modeling,below>>, but first let's look at those data operations, and installing the {name-sdk}.
...
+
--
If you haven't already got a cluster set up, the easiest route is to https://cloud.couchbase.com/sign-up[sign up to Couchbase Capella and deploy a free tier cluster^], then come back to this page.
...
* The Couchbase {name-sdk} {sdk_dot_minor} Client is tested against Rust 1.90.
...
In most cases, given the above prerequisites, it's a simple matter of the following:
...
====
....
include::devguide:example$src/capella_sample.rs[]
....
====
...
NOTE: `Cluster.connect` returns a `Result<Cluster, Error>`.
...
Operations in the {name-sdk} do not support timeouts in order to feel idiomatic to the Rust ecosystem.
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=bucket,indent=0]
----
...
`waitUntilReady` is an optional call, but it is good practice to use it.
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=collection,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=json,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=upsert,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=get,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=get-better-error-handling-propagate,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=get-better-error-handling,indent=0]
----

=== Replace (Update) and Overloads

You'll notice that most operations in the {name-sdk} have an `Options` parameter.
For instance:

.The replace method updates the value of an existing document
[source,rust]
----
include::devguide:example$src/capella_sample.rs[tag=replace-options,indent=0]
----
...
[source,rust]
----
include::devguide:example$src/kv_operations.rs[tag=remove,indent=0]
----

== Data Modeling

Documents are organized into collections -- collections of documents that belong together.
You get to decide what it means to "belong." Developers usually put documents of the same type in the same collection.
...
Each collection belongs to just one scope and a collection's name is unique within the scope.
