...
[source,rust]
----
include::devguide:example$src/data_durability_consistency.rs[tag=remove_with_durability,indent=0]
----
...
xref:howtos:concurrent-document-mutations.adoc[], and try some of the code snippets there.
...
Here we hope to keep it _relatively_ simple, with a look at which Couchbase features give you which guarantees, when you may prefer to use them, and -- perhaps equally important, though sometimes overlooked --
...
The schema-on-read approach allows the client software that you write with the SDK to work with changes to an implicit schema, and allows heterogeneous data.

=== Objects, Relations, Tables

In the Relational Database (RDBMS) world, a translation layer is often used between the objects of your data model in your application, and the tables that you store the data in.
JSON storage allows you to store complex types, like nested records and arrays, without decomposing them to a second table (known in the SQL world as https://en.wikipedia.org/wiki/Database_normalization[database normalization^]).
...
For example, one collection of students enrolled at the college and one collection of courses available for them to take.
...
You can set Durablilty as part of regular CRUD operations against the Data Service, or set it per Bucket from the Server or Capella.
...
A practical look at this -- using the `durability_level` parameter with mutating operations -- can be found on the xref:howtos:kv-operations.adoc#durability[Data Service page].
...
Should you need the guarantees of explicit locking, documents can be locked using the _get-and-lock_ operation and unlocked either explicitly using the _unlock_ operation or implicitly by mutating the document with a valid CAS.
...
For more on Durability, see xref:durability-replication-failure-considerations.adoc[].
